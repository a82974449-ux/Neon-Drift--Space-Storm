...
function resizeCanvas() {
    ...
    // إزالة قيود التكبير لكي تتناسب اللعبة 100% مع التابلت والآيباد
    baseScale = scaleW;

    // تحديث حجم وموقع اللاعب ديناميكياً
    player.size = 22 * baseScale;
    if (!gameActive) player.y = canvas.height - (150 * scaleH);
}
...
// متغير لحساب الزمن الفعلي (Delta Time) لتوحيد السرعة على جميع الشاشات
let lastTime = 0;
...
window.setPause = function(paused) {
    ...
    if (paused && audioCtx && audioCtx.state === 'running') {
        ...
        audioCtx.resume();
        lastTime = performance.now(); // إعادة ضبط الزمن لتجنب القفزات عند المتابعة
    }
};
...
window.initGame = function() {
    ...
    currentShakeX = 0;
    currentShakeY = 0;
    lastTime = performance.now(); // تهيئة زمن البداية
    
    initAudio(); 
};

// 7. حلقة اللعبة المستمرة (مدعومة بنظام Delta Time)
function gameLoop(timestamp) {
    // حساب معدل تحديث الشاشة لتثبيت السرعة
    if (!timestamp) timestamp = performance.now();
    let deltaTime = timestamp - lastTime;
    lastTime = timestamp;
    
    // منع حدوث قفزات كبيرة في الحركة إذا خرج اللاعب من التطبيق وعاد
    if (deltaTime > 100 || deltaTime < 0) deltaTime = 16.666;
    
    // المعامل dt يوازن الحركة: 1.0 للشاشات العادية، 0.5 لشاشات 120Hz السريعة
...
function updateTimers(dt) {
    ...
    for (let i = damageTexts.length - 1; i >= 0; i--) {
        damageTexts[i].y -= 1.2 * scaleH * dt;
        damageTexts[i].alpha -= 0.02 * dt;
        if (damageTexts[i].alpha <= 0) damageTexts.splice(i, 1);
    }
    for (let i = milestoneTexts.length - 1; i >= 0; i--) {
        milestoneTexts[i].y -= 0.3 * scaleH * dt; 
        milestoneTexts[i].alpha -= 0.015 * dt; 
...
function updateObstacles(dt) {
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        if (obs.type === 'zigzag') obs.x += Math.sin(obs.y * 0.04) * obs.zigzagAmplitude * obs.zigzagDirection * dt;
        else if (obs.type === 'satellite') {
            obs.x += obs.speed * 0.5 * obs.satelliteDirection * dt;
            if (obs.x < obs.size || obs.x > canvas.width - obs.size) obs.satelliteDirection *= -1;
        }
        else if (obs.type === 'seeker') {
            obs.x += (player.x - obs.x) * (0.015 + currentLevel * 0.001) * dt; 
        }
...
function updateItems(dt) {
    scoreItemTimer += dt; shieldItemTimer += dt; pulseBombTimer += dt; heartItemTimer += dt;
    ...
    for (let i = items.length - 1; i >= 0; i--) { 
        ...
        if (it.type === 'life') {
            it.wavePhase += 0.08 * dt;
            it.x = it.startX + Math.sin(it.wavePhase) * (60 * scaleW); 
...
function drawDamageTexts() {
    ...
    for (let textObj of damageTexts) {
        let t = textObj.text || '-10'; 
        let c = textObj.color || '255, 51, 51'; 
