...
let items = []; 
let damageTexts = []; 
let milestoneTexts = []; // مصفوفة لتخزين الإشعارات التحفيزية
let obstacleSpawnTimer = 0;
let scoreItemTimer = 0;
let shieldItemTimer = 0; 
let pulseBombTimer = 0; 
let score = 0;
let nextMilestone = 100; // الهدف القادم للإشعار
...
function updateTimers() {
    ...
    for (let i = milestoneTexts.length - 1; i >= 0; i--) {
        milestoneTexts[i].y -= 0.3; // يرتفع لأعلى ببطء شديد
        milestoneTexts[i].alpha -= 0.015; // يتلاشى ببطء شديد
...
function updateObstacles() {
    ...
    if (obstacleSpawnTimer > spawnInterval) {
        ...
        obstacles.push({
            ...
            zigzagAmplitude: Math.random() * 2 + 1.5 + (currentLevel * 0.1), 
...
function drawHighScoreUI() {
    ...
    // رسم الإشعارات التحفيزية
    for (let mt of milestoneTexts) {
        ctx.fillStyle = `rgba(255, 204, 0, ${Math.min(1, mt.alpha)})`; // لون ذهبي معتم
        ctx.shadowColor = '#ffcc00';
        ctx.shadowBlur = 15;
        ctx.font = 'bold 1.2rem sans-serif';
        ctx.fillText(mt.text, 20, mt.y);
    }
